...
class TransformerClassifier(nn.Module):
    def __init__(self, vocab_size, d_model=128, heads=4, layers=3, max_len=256, num_classes=2):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, d_model, padding_idx=0)
        self.pos_enc = PositionalEncoding(d_model, max_len)
        self.layers = nn.Sequential(*[TransformerBlock(d_model, heads) for _ in range(layers)])
